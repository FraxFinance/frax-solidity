...
module.exports = async function (deployer, network, accounts) {
  ...
  await fraxInstance.addPool(pool_instance_USDC.address, {
    from: COLLATERAL_FRAX_AND_FXS_OWNER,
  });
  await fraxInstance.addPool(pool_instance_USDT.address, {
    from: COLLATERAL_FRAX_AND_FXS_OWNER,
  });
  ...
  await fxsInstance.setFRAXAddress(fraxInstance.address, {
    from: COLLATERAL_FRAX_AND_FXS_OWNER,
  });
  ...
  let frax_price_from_FRAX_FXS = new BigNumber(
    await oracle_instance_FRAX_FXS.consult.call(fxsInstance.address, 1e6)
  ).div(BIG6);
  ...
  const frax_price_initial = new BigNumber(
    await fraxInstance.frax_price({from: COLLATERAL_FRAX_AND_FXS_OWNER})
  ).div(BIG6);
  const fxs_price_initial = new BigNumber(
    await fraxInstance.fxs_price({from: COLLATERAL_FRAX_AND_FXS_OWNER})
  ).div(BIG6);

  // Print the new prices
  console.log(
    'frax_price_initial: ',
    frax_price_initial.toString(),
    'USD = 1 FRAX'
  );
  console.log(
    'fxs_price_initial: ',
    fxs_price_initial.toString(),
    'USD = 1 FXS'
  );
  console.log(
  ...
  console.log(
    'frax_price_from_FRAX_FXS: ',
    frax_price_from_FRAX_FXS.toString(),
    'FRAX = 1 FXS'
  );
  console.log(
  ...
  console.log(
    chalk.yellow('===== TRANSFER SOME TOKENS AND ETH TO METAMASK =====')
  );

  // FRAX and FXS
  await Promise.all([
    fxsInstance.transfer(METAMASK_ADDRESS, new BigNumber('1000e18'), {
      from: COLLATERAL_FRAX_AND_FXS_OWNER,
    }),
    fraxInstance.transfer(METAMASK_ADDRESS, new BigNumber('1000e18'), {
      from: COLLATERAL_FRAX_AND_FXS_OWNER,
    }),
  ]);
  ...
  const hasLibraries =
    CONTRACT_ADDRESSES &&
    CONTRACT_ADDRESSES[process.env.MIGRATION_MODE] &&
    CONTRACT_ADDRESSES[process.env.MIGRATION_MODE].libraries;
