...
module.exports = async function (deployer, network, accounts) {
  ...
  console.log(
    chalk.yellow('===== DEPLOY OR LINK THE ROUTER AND SWAP_TO_PRICE =====')
  );
  let routerInstance;
  ...
  if (IS_MAINNET) {
    swapToPriceInstance = await SwapToPrice.at(
      '0xa61cBe7E326B13A8dbA11D00f42531BE704DF51B'
    );
  } else {
    await deployer.deploy(
      SwapToPrice,
  ...
  await deployer.link(FRAXStablecoin, [
    StakingRewards_FRAX_WETH,
    StakingRewards_FRAX_USDC,
    StakingRewards_FXS_WETH,
    StakingRewards_FRAX_FXS,
  ]);
