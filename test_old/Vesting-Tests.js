...
const StakingRewards_FRAX_USDT = artifacts.require(
  'Staking/Fake_Stakes/Stake_FRAX_USDT.sol'
);
...
const StakingRewards_FXS_USDC = artifacts.require(
  'Staking/Fake_Stakes/Stake_FXS_USDC.sol'
);
const StakingRewards_FXS_USDT = artifacts.require(
  'Staking/Fake_Stakes/Stake_FXS_USDT.sol'
);
...
contract('FRAX', async (accounts) => {
  ...
  it('Deposits FXS into the vesting contract, attempts to withdraw before the cliff, between cliff and end, and after end', async () => {
    ...
    console.log(
      'accounts[9] FXS balance:',
      new BigNumber(await fxsInstance.balanceOf(accounts[9]))
        .div(BIG18)
        .toNumber()
    );
    ...
    console.log(
      'released:',
      new BigNumber(await vestingInstance.released()).div(BIG18).toNumber()
    );
