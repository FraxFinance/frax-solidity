...
contract CurveAMO_V2 is AccessControl {
    ...
    modifier onlyCustodianOrVoter() {
        require(
            msg.sender == custodian_address || msg.sender == voter_contract_address,
            "Must be rewards custodian or the voter contract"
        );
        _;
    }
        ...
        returns (uint256 metapool_LP_received)
    {
...

        {
            ...
            uint256 min_lp_out =
                (_frax_amount.add(threeCRV_received)).mul(add_liq_slippage_metapool).div(PRICE_PRECISION);
    ...
    function metapoolWithdrawAtCurRatio(
        ...
        uint256 min_frax,
        uint256 min_3pool
    ) external onlyByOwnerOrGovernance returns (uint256 frax_received) {
        ...
        {
            // Add the FRAX and the collateral to the metapool
            uint256 min_collat_out =
                three_pool_received.mul(liq_slippage_3crv).div(PRICE_PRECISION * (10**missing_decimals));
    ...
    function setThreePool(address _three_pool_address, address _three_pool_token_address)
        external
        onlyByOwnerOrGovernance
    {
        three_pool_address = _three_pool_address;
    ...
    function setSlippages(
        uint256 _liq_slippage_3crv,
        uint256 _add_liq_slippage_metapool,
        uint256 _rem_liq_slippage_metapool
