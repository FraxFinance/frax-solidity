...
contract FraxBondIssuer is AccessControl {
    ...
    modifier onlyByOwnerControllerOrGovernance() {
        require(
            msg.sender == owner_address || msg.sender == timelock_address || msg.sender == controller_address,
    ...
    function floor_price() public view returns (uint256 _floor_price) {
        ...
        _floor_price = (PRICE_PRECISION.sub(initial_discount)).add(
    ...
    function getAmountOut(
        ...
        uint256 amountInWithFee = amountIn.mul(uint256(PRICE_PRECISION).sub(the_fee));
        uint256 numerator = amountInWithFee.mul(reserveOut);
        uint256 denominator = (reserveIn.mul(PRICE_PRECISION)).add(amountInWithFee);
        ...
        )
    {
        ...
        if (fxb_sold_under_floor > 0) {
            ...
            (uint256 frax_floor_balance_virtual, uint256 fxb_floor_balance_virtual) =
    ...
    function expand_AMM_liquidity(uint256 fxb_expansion_amount, bool do_rebalance)
        external
    ...
    // The expansion must occur at the current vAMM price
    function contract_AMM_liquidity(uint256 fxb_contraction_amount, bool do_rebalance)
        external
        onlyByOwnerControllerOrGovernance
    {
        require(isInEpoch(), "Not in an epoch");
    ...
    function setTargetLiquidity(uint256 _target_liquidity_fxb, bool _rebalance_vAMM)
        external
        onlyByOwnerControllerOrGovernance
    {
