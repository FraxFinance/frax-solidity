...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    constructor(
        ...
        ERC721(
            FraxBond_NFT_Library.fxb_namer(_series, _face_value, _maturity_months),
            FraxBond_NFT_Library.fxb_symboler(_series, _face_value, _maturity_months)
        )
    {
        ...
        returns (uint256 maturity_value, uint256 return_value)
    {
        ...
        maturity_value = maturity_value.sub(
            maturity_value.mul(penalty_multiplier).mul(max_early_redemption_penalty_pct).div(1e12)
        );
