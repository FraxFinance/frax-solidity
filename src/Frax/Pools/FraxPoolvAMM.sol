...
contract FraxPoolvAMM is AccessControl {
    ...
    function getOracleInfo() public view returns (uint256 ext_average_fxs_usd_price, uint256 ext_k) {
    ...
    function collatDollarBalance() public view returns (uint256) {
        return
            (collateral_token.balanceOf(address(this)).add(collateral_invested).sub(unclaimedPoolCollateral)).mul(
                10**missing_decimals
            );
    }
    ...
    function availableForInvestment() public view returns (uint256 max_invest) {
        uint256 curr_pool_bal =
            collateral_token.balanceOf(address(this)).add(collateral_invested).sub(unclaimedPoolCollateral);
        ...
        )
    {
        ...
        if (global_collateral_ratio == 1e6) {
            ...
            collat_needed = frax_mint_from_fxs.mul(1e6).div(uint256(1e6).sub(global_collateral_ratio)); // find collat needed at collateral ratio
        ...
        require(
            collateral_token.balanceOf(address(this)).add(collateral_invested).sub(unclaimedPoolCollateral).add(
                collat_needed
            ) <= pool_ceiling,
        ...
        require(
            total_frax_mint <=
                collateral_amount.mul(10**missing_decimals).mul(uint256(1e6).add(max_drift_band)).div(
        ...
        )
    {
        ...
        if (global_collateral_ratio == 1e6) {
            ...
            fxs_out = getAmountOut(
                collat_equivalent.mul((uint256(1e6).sub(global_collateral_ratio))).div(1e6),
                collat_virtual_reserves,
                fxs_virtual_reserves,
                redemption_fee
            );
        ...
        require(
            collat_out <= collateral_token.balanceOf(address(this)).sub(unclaimedPoolCollateral),
        ...
        require(
            collat_out.mul(10**missing_decimals) <=
                FRAX_amount.mul(global_collateral_ratio).mul(uint256(1e6).add(max_drift_band)).div(1e12),
    ...
    function recollateralizeFRAX(uint256 collateral_amount, uint256 FXS_out_min) external returns (uint256, uint256) {
        ...
        require(
            target_collat_value >= global_collat_value + collateral_amount.mul(10**missing_decimals),
        ...
        require(
            fxs_out.mul(fxs_price).div(1e6) <=
                collateral_amount.mul(10**missing_decimals).mul(uint256(1e6).add(max_drift_band)).div(1e6),
    ...
    function buyBackFXS(uint256 FXS_amount, uint256 COLLATERAL_out_min) external returns (uint256, uint256) {
        ...
        require(
            collat_out.mul(10**missing_decimals) <=
                FXS_amount.mul(fxs_price).mul(uint256(1e6).add(max_drift_band)).div(1e12),
            "[max_drift_band] Too much collateral being released"
        );
    ...
    function setInvestorParameters(address _investor_contract_address, uint256 _global_investment_cap_percentage)
        external
