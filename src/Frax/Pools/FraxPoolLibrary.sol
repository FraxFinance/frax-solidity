...
library FraxPoolLibrary {
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params) internal pure returns (uint256, uint256) {
        ...
        uint256 calculated_fxs_needed = calculated_fxs_dollar_value_d18.mul(1e6).div(params.fxs_price_usd);
    ...
    function calcRecollateralizeFRAXInner(
        ...
        uint256 recollat_possible =
            (global_collateral_ratio.mul(frax_total_supply).sub(frax_total_supply.mul(effective_collateral_ratio))).div(
                1e6
            );
