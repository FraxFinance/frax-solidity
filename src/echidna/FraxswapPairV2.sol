...
contract FraxswapPair is IUniswapV2PairPartialV5, FraxswapERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1, uint32 timeElapsed) private {
        if (!(balance0 + twammReserve0 <= type(uint112).max && balance1 + twammReserve1 <= type(uint112).max)) revert("EC02"); // OVERFLOW
        uint32 blockTimestamp = uint32(block.timestamp % 2 ** 32);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock execVirtualOrders {
        if(!(amount0Out > 0 || amount1Out > 0)) revert("EC03"); // INSUFFICIENT_OUTPUT_AMOUNT
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        if(!(amount0Out < _reserve0 && amount1Out < _reserve1)) revert("EC04"); // INSUFFICIENT_LIQUIDITY
        ...
        {// scope for _token{0,1}, avoids stack too deep errors
            ...
            if(!(to != _token0 && to != _token1)) revert("EC05"); // INVALID_TO
        ...
        if(!(amount0In > 0 || amount1In > 0)) revert("EC06"); // INSUFFICIENT_INPUT_AMOUNT
        {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
            ...
            if (!(balance0Adjusted * balance1Adjusted >= uint(_reserve0) * _reserve1 * (10000 ** 2))) revert("K"); // K
        }
...
contract EchidnaTest is FraxswapPair {
    ERC20 t0;
    ERC20 t1;
    ...
    function initialize(uint112 _totalSupply0, uint112 _totalSupply1, uint112 liquidity0, uint112 liquidity1) public {
        ...
        try this.initialize(address(t0), address(t1), 30) {} catch { assert(false); } // Poscondition: initialization should never revert
    ...
    function testLiquidity() public {
        (uint112 _reserve0, uint112 _reserve1,, uint112 _twammReserve0, uint112 _twammReserve1, uint256 _fee) = getTwammReserves();
        ...
        emit Value("testLiquidity t0.balance:", t0Balance);
        emit Value("testLiquidity t1.balance:", t1Balance);
        // Global invariant: minimal liquidity should be preserved for both tokens
        // I don't think this test is correct! MINIMUM_LIQUIDITY is the LP TOKENS
        assert(!addedMinLiquidity || _reserve0 + _twammReserve0 <= t0Balance);
        assert(!addedMinLiquidity || _reserve1 + _twammReserve1 <= t1Balance);
    }
    ...
    function changeFee(uint fee) external {
        this.setFee(fee);
    }
    function changeFee01() external {
        this.setFee(1);
    }
    function changeFee05() external {
        this.setFee(5);
    }
    function changeFee10() external {
        this.setFee(10);
    }
    function changeFee20() external {
        this.setFee(20);
    }
    function changeFee30() external {
        this.setFee(30);
    }
    function changeFee40() external {
        this.setFee(40);
    }
    function changeFee50() external {
        this.setFee(50);
    }
    function changeFee60() external {
        this.setFee(60);
    }
    function changeFee70() external {
        this.setFee(70);
    }
    function changeFee80() external {
        this.setFee(80);
    }
    function changeFee90() external {
        this.setFee(90);
    }
    function changeFeeMax() external {
        this.setFee(100);
    }
