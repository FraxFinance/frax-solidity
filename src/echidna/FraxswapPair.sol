...
library ExecVirtualOrdersLib {
    ...
    internal /*pure*/ returns (uint256 token0Out, uint256 token1Out)
    {
        ...
        emit Value("token1Start", token1Start);
        emit Value("token0In", token0In);
        emit Value("token1In", token1In);
        token0Out = 0;
        token1Out = 0;
        ...
        //when both pools sell, we use the TWAMM formula
            ...
            emit Value("token0Start", token0Start);
//            emit Value("aIn", (token0In * 997 / 1000));
//            emit Value("ammEndToken0", token0Start - token0Out);
        }
    }
}
