...
contract FraxFarm_UniV3_veFXS is Owned, ReentrancyGuard {
    ...
    modifier onlyByOwnerOrGovernanceOrMigrator() {
        require(
            msg.sender == owner || msg.sender == timelock_address || valid_migrators[msg.sender] == true,
    ...
    function checkUniV3NFT(uint256 token_id, bool fail_if_false)
        public
        view
        returns (bool is_valid, uint256 liquidity)
    {
    ...
    function _stakeLocked(
        ...
        require(
            (stakingPaused == false && migrationsOn == false) || valid_migrators[msg.sender] == true,
        ...
        uint256 combined_weight_to_add;
        {
            ...
            combined_weight_to_add = liquidity
                .mul(lock_multiplier)
                .mul(_vefxsMultiplierStored[staker_address])
                .div(PRICE_PRECISION)
                .div(VEFXS_MULTIPLIER_PRECISION);
            lockedNFTs[staker_address].push(
                LockedNFT(token_id, liquidity, block.timestamp, block.timestamp.add(secs), lock_multiplier)
            );
        }
