...
contract StakingRewardsDualV2 is IStakingRewardsDual, Owned, ReentrancyGuard, Pausable {
    ...
    function stakingMultiplier(uint256 secs) public view returns (uint256) {
        uint256 multiplier =
            uint256(MULTIPLIER_BASE).add(
                secs.mul(locked_stake_max_multiplier.sub(MULTIPLIER_BASE)).div(locked_stake_time_for_max_multiplier)
            );
    ...
    function earned(address account) public view override returns (uint256, uint256) {
        (uint256 reward0, uint256 reward1) = rewardPerToken();
        return (
            _boosted_balances[account].mul(reward0.sub(userRewardPerTokenPaid0[account])).div(1e18).add(
                rewards0[account]
            ),
            _boosted_balances[account].mul(reward1.sub(userRewardPerTokenPaid1[account])).div(1e18).add(
                rewards1[account]
            )
        );
    }

    function getRewardForDuration() external view override returns (uint256, uint256) {
        return (
    ...
    function _stakeLocked(
        ...
        require(
            ...
            StringHelpers.strConcat("Minimum stake time not met (", locked_stake_min_time_str, ")")
        );
    ...
    function retroCatchUp() internal {
        ...
        require(
            rewardRate0.mul(rewardsDuration).mul(crBoostMultiplier()).mul(num_periods_elapsed + 1).div(
                PRICE_PRECISION
            ) <= balance0,
    ...
    function setMultipliers(uint256 _locked_stake_max_multiplier, uint256 _cr_boost_max_multiplier)
        external
