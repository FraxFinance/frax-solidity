...
contract FXSRewards {
    ...
    function fxsDollarValueStaked() public view returns (uint256) {
        return
            FXS.balanceOf(address(veFXS)).mul(uint256(fxs_usd_pricer.getLatestPrice())).div(
                10**fxs_usd_pricer_decimals
            );
    }
    ...
    function swapCollateral(uint256 frax_amount) public onlyByOwnerOrGovernance returns (uint256) {
        uint256 received_collat =
            frax3crv_metapool.exchange_underlying(
                FRAX_INDEX,
                USDC_INDEX,
                frax_amount,
                frax_amount.mul(curve_slippage).div(1e6)
            );
    ...
    function returnCollateral(uint256 collateral_amount) public onlyByOwnerOrGovernance returns (uint256) {
        uint256 received_frax =
            frax3crv_metapool.exchange_underlying(
                USDC_INDEX,
                FRAX_INDEX,
                collateral_amount,
                collateral_amount.mul(curve_slippage).div(1e6)
            );
