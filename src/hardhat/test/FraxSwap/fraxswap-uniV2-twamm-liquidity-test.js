...
describe("UniswapV2 Tests", function () {
    ...
    describe("Core Tests - UniswapV2PairV8", function () {
        ...
        it('swap:token0', async () => {
            ...
            expect(await token0.balanceOf(user1.address)).to.eq(totalSupplyToken0.sub(user2Tokens).sub(token0Amount).sub(swapAmount))
            expect(await token1.balanceOf(user1.address)).to.eq(totalSupplyToken1.sub(user2Tokens).sub(token1Amount).add(expectedOutputAmount))
        });
        ...
        it('swap:token1', async () => {
            ...
            const totalSupplyToken0 = await token0.totalSupply()
            const totalSupplyToken1 = await token1.totalSupply()
            expect(await token0.balanceOf(user1.address)).to.eq(totalSupplyToken0.sub(user2Tokens).sub(token0Amount).add(expectedOutputAmount))
            expect(await token1.balanceOf(user1.address)).to.eq(totalSupplyToken1.sub(user2Tokens).sub(token1Amount).sub(swapAmount))
        });
        ...
        it('burn', async () => {
            ...
            expect((await token0.balanceOf(pair.address)).sub(longTermOrderAmount)).to.eq(1000)
            expect(await token1.balanceOf(pair.address)).to.eq(1000)
            const totalSupplyToken0 = await token0.totalSupply()
            const totalSupplyToken1 = await token1.totalSupply()
            expect(await token0.balanceOf(user1.address)).to.eq(totalSupplyToken0.sub(user2Tokens).sub(1000))
            expect(await token1.balanceOf(user1.address)).to.eq(totalSupplyToken1.sub(user2Tokens).sub(1000))
        });
        ...
        it('price{0,1}CumulativeLast', async () => {
            ...
            console.log(`reserve3: ${await pair.getReserves()}`)
        ...
        it('feeTo:on', async () => {
            ...
            expect((await token0.balanceOf(pair.address)).sub(longTermOrderAmount)).to.eq(bigNumberify(1000).add('249501683697445'))
    ...
    describe("Core Tests - UniswapV2FactoryV8", function () {
        ...
        it('createPair:gas', async () => {
            ...
            expect(receipt.gasUsed).to.eq(5127352)
        })
...
async function setupContracts(createPair = true, deployDTT = false) {
    ...
    if (deployDTT) {
        ...
        await deflatingERC20.transfer(user1.address, user1Tokens);
    }
    ...
    await token0.transfer(user1.address, user1Tokens);
    await token1.transfer(user1.address, user1Tokens);
    await token0.transfer(user2.address, user2Tokens);
    await token1.transfer(user2.address, user2Tokens);
