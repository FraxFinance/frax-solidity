...
contract('FraxGaugeControllerAndUniV3Farm-Tests', async (accounts) => {
	...
	it('Initialize FraxGaugeController', async () => {
		...
		await frax_gauge_controller.add_gauge(fraxFarmInstance_FRAX_USDC.address, 0, 2000, { from: DEPLOYER_ADDRESS });

		// Add the FRAX-WETH contract as a gauge (to take up weight)
		await frax_gauge_controller.add_gauge(stakingInstance_FRAX_WETH.address, 0, 500, { from: DEPLOYER_ADDRESS });
	...
	it('Initialization related', async () => {
		...
		await fxsInstance.transfer(fraxFarmInstance_FRAX_USDC.address, new BigNumber("1000000e18"), { from: ADDRESS_WITH_FXS });
	...
	it('Locked stakes', async () => {
		...
		const fxs_balance_before_reward = new BigNumber(await fxsInstance.balanceOf.call(accounts[9])).div(BIG18);
		...
		const fxs_balance_after_reward = new BigNumber(await fxsInstance.balanceOf.call(accounts[9])).div(BIG18);
		...
		// await fxsInstance.transfer(fraxFarmInstance_FRAX_USDC.address, new BigNumber("100000e18"), { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		// console.log("stakingInstance FXS balance:", new BigNumber(await fxsInstance.balanceOf(fraxFarmInstance_FRAX_USDC.address)).div(BIG18).toNumber());
		// await fraxFarmInstance_FRAX_USDC.getReward({ from: COLLATERAL_FRAX_AND_FXS_OWNER });
	...
	it("Fail Tests ", async () => {
		...
		await uniswapV3PositionsNFTInstance.approve(fraxFarmInstance_FRAX_USDC.address, TOKEN_ID_1, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		await expectRevert(
			fraxFarmInstance_FRAX_USDC.stakeLocked(TOKEN_ID_1, 28 * 86400, { from: COLLATERAL_FRAX_AND_FXS_OWNER }),
