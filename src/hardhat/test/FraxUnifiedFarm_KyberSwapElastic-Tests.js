...
const ComboOracle_KyberSwapElastic = artifacts.require("contracts/Oracle/ComboOracle_KyberSwapElastic");
...
contract('FraxUnifiedFarm_KyberSwapElastic-Tests', async (accounts) => {
	...
	const ADDRESS_WITH_FRAX = '0x94A84433101A10aEda762968f6995c574D1bF154';
	...
	const ADDRESS_WITH_KNC = '0x09D51654bd9eFbfCB56da3491989Cc1444095fFf';
	const ADDRESS_WITH_VEFXS = '0x5180db0237291A6449DdA9ed33aD90a38787621c';
	const ADDRESS_VEFXS_WHALE = '0xe0dd07B181cA77eD9063821b4D2D28a85f722f5f';

	const ADDRESS_WITH_UNI_V3_NFT_1 = '0x5180db0237291A6449DdA9ed33aD90a38787621c';
	const TOKEN_ID_1 = 301;
	const TOKEN_ID_1_ALT_GOOD = 303;
	const ADDRESS_WITH_UNI_V3_NFT_9 = '0x36a87d1e3200225f881488e4aeedf25303febcae';
	const TOKEN_ID_9_INVALID_WRONG_TICKS = 308;
	const TOKEN_ID_9_INVALID_WRONG_UNDERLYING_TOKENS = 309;
	const TOKEN_ID_9_INVALID_WRONG_FEE = 307;
	const TOKEN_ID_9 = 304;
	const TOKEN_ID_9_INSIDE_WRONG_TICKS = 305;
	const TOKEN_ID_9_OUTSIDE_WRONG_TICKS = 306;
    ...
    beforeEach(async() => {
		...
		knc_instance = await ERC20.at("0xdeFA4e8a7bcBA345F687a2f1456F5Edd9CE97202");

		// Fill the staking rewards instances
		fraxUnifiedFarm_KyberSwapElastic_instance = await FraxUnifiedFarm_KyberSwapElastic.deployed();
		frax_is_token0 = await fraxUnifiedFarm_KyberSwapElastic_instance.frax_is_token0.call();
		
		// Fill the combo oracle
		combo_oracle_instance = await ComboOracle.at(CONTRACT_ADDRESSES.ethereum.oracles_other.combo_oracle);
		combo_oracle_kyberswap_elastic_instance = await ComboOracle_KyberSwapElastic.deployed();

		// Fill the Uniswap V3 Instances
		kyberNFTPositionMgr = await IAntiSnipAttackPositionManager.at(CONTRACT_ADDRESSES.ethereum.uniswap_v3.kyberswap_elastic_pos_mgr); 

		// veFXS and gauge related
		veFXS_instance = await veFXS.deployed();
		frax_gauge_controller = await FraxGaugeController.at(CONTRACT_ADDRESSES.ethereum.misc.frax_gauge_controller_v2);
		...
		staking_instance = fraxUnifiedFarm_KyberSwapElastic_instance;
	...
	it('Main test', async () => {
		...
		const seed_nft_id = await staking_instance.seed_token_id.call();
		await utilities.printNFTExtendedInfo(combo_oracle_kyberswap_elastic_instance, seed_nft_id);
		...
		console.log("------------------------------------------------");
		console.log("Seed the staking contract with KNC");
		await hre.network.provider.request({
			method: "hardhat_impersonateAccount",
			params: [ADDRESS_WITH_KNC]
		});

		await knc_instance.transfer(staking_instance.address, new BigNumber("1000e18"), { from: ADDRESS_WITH_KNC });
		...
		console.log("------------------------------------------------");
		console.log("Give address[1] some USDC");
		await hre.network.provider.request({
			method: "hardhat_impersonateAccount",
			params: [ADDRESS_WITH_USDC]
		});

		await usdc_instance.transfer(COLLATERAL_FRAX_AND_FXS_OWNER, new BigNumber("3000e6"), { from: ADDRESS_WITH_USDC });
		...
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_1, accounts[1], TOKEN_ID_1, { from: ADDRESS_WITH_UNI_V3_NFT_1 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_1, accounts[1], TOKEN_ID_1_ALT_GOOD, { from: ADDRESS_WITH_UNI_V3_NFT_1 });
		...
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9_INVALID_WRONG_TICKS, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9_INVALID_WRONG_UNDERLYING_TOKENS, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9_INVALID_WRONG_FEE, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9_INSIDE_WRONG_TICKS, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		await kyberNFTPositionMgr.safeTransferFrom(ADDRESS_WITH_UNI_V3_NFT_9, accounts[9], TOKEN_ID_9_OUTSIDE_WRONG_TICKS, { from: ADDRESS_WITH_UNI_V3_NFT_9 });
		...
		// Allow stale oracle reads for this test (prod is 7 days)
		await combo_oracle_instance.setMaxDelayTime(157680000, { from: COMPTROLLER_ADDRESS }); // 5 years
	...
	it('Locked stakes', async () => {
		...
		const nft_info = await kyberNFTPositionMgr.positions.call(TOKEN_ID_1);
		const liq_locked_1 = new BigNumber(nft_info[0][5]);
		...
		await kyberNFTPositionMgr.approve(staking_instance.address, TOKEN_ID_1, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		// account[1]
		await kyberNFTPositionMgr.approve(staking_instance.address, TOKEN_ID_1_ALT_GOOD, { from: COLLATERAL_FRAX_AND_FXS_OWNER });

		// account[9]
		await kyberNFTPositionMgr.approve(staking_instance.address, TOKEN_ID_9, { from: accounts[9] });
		...
		// Stake Locked
		// account[1]
		await staking_instance.stakeLocked(TOKEN_ID_1_ALT_GOOD, 365 * 86400, { from: COLLATERAL_FRAX_AND_FXS_OWNER }); // 365 days
		...
		const add_tkn_amt_e6 = new BigNumber("1000e6");
		await usdc_instance.approve(staking_instance.address, add_tkn_amt_e6, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		await frax_instance.approve(staking_instance.address, add_tkn_amt, { from: COLLATERAL_FRAX_AND_FXS_OWNER });

		// Lock additional
		if (frax_is_token0){
			await staking_instance.lockAdditional(TOKEN_ID_1, add_tkn_amt, add_tkn_amt_e6, 0, 0, false, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		}
		else {
			await staking_instance.lockAdditional(TOKEN_ID_1, add_tkn_amt_e6, add_tkn_amt, 0, 0, false, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		}
		...
		const add_tkn_balof_ovr_amt_e6 = new BigNumber("1000e6");
		await usdc_instance.transfer(staking_instance.address, add_tkn_balof_ovr_amt_e6, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		await frax_instance.transfer(staking_instance.address, add_tkn_balof_ovr_amt, { from: COLLATERAL_FRAX_AND_FXS_OWNER });

		// Lock additional
		// Uses balanceOf override to pull in all of the user's tokens
	...
	it("Communal / Token Manager Tests", async () => {
		...
		// Get some balances
		const knc_bal = new BigNumber(await knc_instance.balanceOf(staking_instance.address)).div(BIG18).toNumber();
	...
	it("Fail Tests ", async () => {
		...
		await expectRevert(
			staking_instance.recoverERC20(knc_instance.address, test_amount_1, { from: STAKING_OWNER }),
