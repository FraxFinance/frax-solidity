...
// Misc AMOs
const StakeDAO_AMO = artifacts.require("Misc_AMOs/StakeDAO_AMO.sol");
...
contract('StakeDAO AMO Tests', async (accounts) => {
    ...
    beforeEach(async() => {
		...
		const StakeDAO_AMO_Implementation = await hre.ethers.getContractFactory("StakeDAO_AMO");
		const proxy_obj = await hre.upgrades.deployProxy(StakeDAO_AMO_Implementation, [
		...
		console.log("StakeDAO_AMO proxy deployed at: ", proxy_instance.address);

		// Get out of ethers and back to web3. It gives a signer-related error
		stakedao_amo_instance = await StakeDAO_AMO.at(proxy_instance.address);

		// // If truffle-fixture is used
		// stakedao_amo_instance = await StakeDAO_AMO.deployed();
	...
	it("Main test", async () => {
		...
		// await stakedao_controller_instance.setStrategy("0xd632f22692fac7611d2aa1c0d552930d43caed3b", "0xEDf53517b700383fcA4f823DCE82eF14cAAe2Ea5", { from: STAKE_DAO_STRATEGIST });
		...
		console.log("Set some variables on the StakeDAO AMO");
		await stakedao_amo_instance.setCollatBorrowCap(new BigNumber("100000000e6"), { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		try {
			...
			await fraxInstance.addPool(stakedao_amo_instance.address, { from: ORIGINAL_FRAX_DEPLOYER_ADDRESS });
		}
		catch(err){
		...
		await stakedao_amo_instance.mintRedeemPart1(new BigNumber("580000e18"), { from: COLLATERAL_FRAX_AND_FXS_OWNER });

		console.log("Get some USDC via the mint method [Part 2]");
		await time.increase(20);
		await time.advanceBlock();
		await time.advanceBlock();
		await stakedao_amo_instance.mintRedeemPart2({ from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		console.log("pre-deposit StakeDAO AMO USDC balance:", new BigNumber(await col_instance_USDC.balanceOf(stakedao_amo_instance.address)).div(BIG6).toNumber());
		console.log("");
		...
		console.log("post-deposit StakeDAO AMO USDC balance:", new BigNumber(await col_instance_USDC.balanceOf(stakedao_amo_instance.address)).div(BIG6).toNumber());
		...
		console.log("");
		console.log("StakeDAO AMO collatDollarBalance():", new BigNumber(await stakedao_amo_instance.collatDollarBalance()).div(BIG18).toNumber());
		...
		const fxs_balance_after = new BigNumber(await fxsInstance.balanceOf.call(stakedao_amo_instance.address)).div(BIG18);
		const usdc_balance_after = new BigNumber(await col_instance_USDC.balanceOf.call(stakedao_amo_instance.address)).div(BIG6);
		const borrowed_balance = new BigNumber(await stakedao_amo_instance.collateralBalance.call()).div(BIG6);
		...
		const big_test_collatdollarbalance_before = new BigNumber(await stakedao_amo_instance.collatDollarBalance()).div(BIG18).toNumber();
		console.log("CurveAMO_V3 collatDollarBalance():", big_test_collatdollarbalance_before);

		console.log("=====================PUT 10M FRAX3CRV IN THE VAULT=====================");
		...
		const fxs_balance_4_after = new BigNumber(await fxsInstance.balanceOf.call(stakedao_amo_instance.address)).div(BIG18);
		const usdc_balance_4_after = new BigNumber(await col_instance_USDC.balanceOf.call(stakedao_amo_instance.address)).div(BIG6);
		const borrowed_balance_4 = new BigNumber(await stakedao_amo_instance.collateralBalance.call()).div(BIG6);
		...
		console.log("CurveAMO_V3 collatDollarBalance():", big_test_collatdollarbalance_after);
