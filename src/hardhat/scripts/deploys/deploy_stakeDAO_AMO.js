...
async function main() {
	...
	// ==================== PROXY DEPLOYMENT ====================
  	const StakeDAO_AMO_V2 = await ethers.getContractFactory("StakeDAO_AMO_V2");
  	const stakeDAO_proxy = await upgrades.deployProxy(StakeDAO_AMO_V2, [
		CONTRACT_ADDRESSES.ethereum.main.FRAX, 
		CONTRACT_ADDRESSES.ethereum.main.FXS, 
		CONTRACT_ADDRESSES.ethereum.collateral.USDC_V2, 
		COLLATERAL_FRAX_AND_FXS_OWNER, 
  	...
  	await stakeDAO_proxy.deployed();
  	console.log("StakeDAO AMO deployed to:", stakeDAO_proxy.address);
}
