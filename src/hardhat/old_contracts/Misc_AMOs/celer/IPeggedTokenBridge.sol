...
interface IPeggedTokenBridge {
    ...
    function maxBurn(address) external view returns (uint256);
    function minBurn(address) external view returns (uint256);
    function mint(bytes memory _request, bytes[] memory _sigs, address[] memory _signers, uint256[] memory _powers) external;
    ...
    function setMaxBurn(address[] memory _tokens, uint256[] memory _amounts) external;
    function setMinBurn(address[] memory _tokens, uint256[] memory _amounts) external;
