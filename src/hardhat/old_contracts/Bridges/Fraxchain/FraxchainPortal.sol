...
import { SafeCall } from "@eth-optimism/contracts-bedrock/contracts/libraries/SafeCall.sol";
...
/// @notice The FraxchainPortal is a low-level contract responsible for passing messages between L1
///         and L2. Messages sent directly to the FraxchainPortal have no form of replayability.
///         Users are encouraged to use the L1CrossDomainMessenger for a higher-level interface.
contract FraxchainPortal is Initializable, ResourceMetering, Semver {
    ...
    address immutable public FRXETH;
    
    address immutable public FRXETH_MINTER;
    ...
    /// @param _frxeth Address of frxETH
    /// @param _frxeth_minter Address of the frxETH minter
    constructor(
    ...
    function pause() external {
        require(msg.sender == GUARDIAN, "FraxchainPortal: only guardian can pause");
        paused = true;
    ...
    function unpause() external {
        require(msg.sender == GUARDIAN, "FraxchainPortal: only guardian can unpause");
        paused = false;
    ...
    function proveWithdrawalTransaction(
        ...
        require(
            ...
            "FraxchainPortal: you cannot send messages to the portal contract"
        );
    ...
    {
        if (msg.value>0) {
            require(msg.value<=_tx.value,"FraxchainPortal: can only swap as much frxETH as is bridged");
            require(IERC20(FRXETH).transfer(msg.sender,msg.value),"FraxchainPortal: frxETH transfer failed");
        }
        require (_tx.target==FRXETH || _tx.value<=address(this).balance,"FraxchainPortal: not enough ETH in the contract");
        ...
        require(
            provenWithdrawal.timestamp != 0,
            "FraxchainPortal: withdrawal has not been proven yet"
        );
        ...
        bool success;
        if (_tx.target==FRXETH) { // Bridge frxETH back to L1
           require(_tx.data.length==20,"FraxchainPortal: invalid recipient specified");
           address recipient = address(uint160(bytes20(_tx.data))); // recipient is stored in the data field
           require(IERC20(FRXETH).transfer(recipient,_tx.value),"FraxchainPortal: frxETH transfer failed");
           success = true;
        } else {
    ...
    function depositTransactionWithFrxETH(
        ...
        require(IERC20(FRXETH).transferFrom(msg.sender, address(this), _value),"FraxchainPortal: frxETH transfer failed");
    ...
    function depositTransaction_internal(
        ...
        if (_isCreation) {
            require(
                _to == address(0),
                "FraxchainPortal: must send to address(0) when creating a contract"
            );
        }
        ...
        require(
            ...
            "FraxchainPortal: gas limit too small"
        );
