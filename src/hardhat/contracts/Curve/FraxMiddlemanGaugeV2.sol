...
contract FraxMiddlemanGauge is Owned, ReentrancyGuard {
    ...
    function pullAndBridge(uint256 reward_amount) external onlyRewardsDistributor nonReentrant {
        ...
        // }
            ...
            /// Familial Gauges (vote balancer between multiple gauges on mainnet)
            /**
            *    Obtain the TVL per gauge
            *    Divide up reward_amount by each gauge's tvl
            *    Send each gauge their share of the reward
            */
            uint256 total_tvl;
            uint256[] memory tvls = new uint256[](gauges.length);
            for (uint256 i; i < gauges.length; i++) {
                uint256 gauge_liquidity = IFraxFarm(gauges[i]).totalLiquidityLocked();
                total_tvl += gauge_liquidity;
                tvls[i] = gauge_liquidity;
            }
            // now we can calculate the reward per gauge and send it
            for (uint256 i; i < gauges.length; i++) {
                uint256 reward_for_gauge = reward_amount * tvls[i] / total_tvl;
                TransferHelper.safeTransfer(reward_token_address, gauges[i], reward_for_gauge);
            }
        }
    ...
    function setBridgeInfo(address _bridge_address, uint256 _bridge_type, address _destination_address_override, string memory _non_evm_destination_address) external onlyByOwnGov {
        ...
        // 5: Harmony
        // 6: Arbitrum
        // 7: Familial Gauges (not a bridge, reward balancer for transferrable locking)
        bridge_type = _bridge_type;
